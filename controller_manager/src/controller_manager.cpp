  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  for (auto loaded_controller : rt_controller_list)
  {
    ...
    if (is_controller_active(*loaded_controller.c))
    {
      ...
      unsigned int update_ratio;
      if (controller_update_rate == 0)
        update_ratio = 1;
      else
        update_ratio = (unsigned int) update_rate_/controller_update_rate;
  
      if (update_ratio < 1 )
        update_ratio = 1;
        
      bool controller_go =
        controller_update_rate == 0 || ((update_loop_counter_ % update_ratio) == 0);
      RCLCPP_DEBUG(
